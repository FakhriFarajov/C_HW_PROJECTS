...
function UseContextSearch() {
    ...
    return (
        <>
            <Nav />
            <Link to="/main" className="flex items-center justify-center flex-col">
                <SearchContext.Provider value={setMovies}>
                    <div className='flex items-center justify-center flex-col'>
                        <SearchForm />
                        <div className='grid grid-cols-3 gap-2'>
                            {movies.results ? movies.results.map(m =>
                                <div key={m.id} className="relative flex flex-col my-5 bg-white shadow-sm border border-slate-200 rounded-lg w-72">
