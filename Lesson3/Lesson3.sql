...
where (select SUM(Departments.Financing) from Departments) > 100000

--2 Ambiguous
select Groups.Name as 'GroupsName' from Groups
...
group by G.Name
having avg(S.Rating) > (
    select avg(S1.Rating)
    from Students S1
    join GroupsStudents GS1 on S1.Id = GS1.StudentId
    join Groups G1 on GS1.GroupId = G1.Id
    where G1.Name = 'D221'
);
